...
public class Keyboard {
    ...
    public void type(CharSequence characters) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if ((character == '^' || character == '%') && i + 1 < length && characters.charAt(i + 1) == '(') {
                List<Integer> chars = new ArrayList<Integer>();
                i++;
                while (i + 1 < length && characters.charAt(i + 1) != ')') {
                    boolean escaped = false;
                    char chr = characters.charAt(++i);
                    if (chr == '\\') {
                        chr = characters.charAt(++i);
                        escaped = true;
                    }
                    int[] keycodes = convert(chr, escaped);
                    for (int keycode : keycodes) {
                        chars.add(keycode);
                    }
                }
                i++;
                int keycodes[] = new int[chars.size() + 1];
                keycodes[0] = character == '^' ? VK_CONTROL : java.awt.event.KeyEvent.VK_ALT;
                for (int j = 1; j < keycodes.length; j++) {
                    keycodes[j] = chars.get(j - 1);
                }
                doType(keycodes);
            } else {
                boolean escaped = false;
    ...
    public int[] convert(char character, boolean escaped) {
        if (escaped) {
            if (character == 'n') {
        ...
        if (character >= 'a' && character <= 'z') {
            return new int[] { VK_A + (character - 'a') };
        }
        if (character >= 'A' && character <= 'Z') {
            return new int[] { VK_SHIFT, VK_A + (character - 'A') };
        }
        if (character >= '0' && character <= '9') {
            return new int[] { VK_0 + (character - '0') };
        }
        //
        //        switch (character) {
        //        case '`':
